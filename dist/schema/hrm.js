import { pgTable, uuid, varchar, text, timestamp, index, uniqueIndex, boolean, foreignKey, date, decimal, integer, pgEnum, } from 'drizzle-orm/pg-core';
import { z } from 'zod';
 ...
 */
export const positions = pgTable('hrm_positions', {
 ...
 * Company Holidays
 */
export const holidays = pgTable('hrm_holidays', {
    id: uuid('id').primaryKey().defaultRandom().notNull(),
    name: varchar('name', { length: 255 }).notNull(),
 ...
 * PTO / Leave Types
 */
export const leaveTypes = pgTable('hrm_leave_types', {
    id: uuid('id').primaryKey().defaultRandom().notNull(),
    name: varchar('name', { length: 255 }).notNull(),
...
export const ptoPolicies = pgTable('hrm_pto_policies', {
    id: uuid('id').primaryKey().defaultRandom().notNull(),
    name: varchar('name', { length: 255 }).notNull(),
    code: varchar('code', { length: 50 }),
    description: text('description'),
 ...
 */
export const ptoPolicyAssignments = pgTable('hrm_pto_policy_assignments', {
    id: uuid('id').primaryKey().defaultRandom().notNull(),
...
export const ptoRequests = pgTable('hrm_pto_requests', {
    id: uuid('id').primaryKey().defaultRandom().notNull(),
 ...
 * PTO Balances
 */
export const ptoBalances = pgTable('hrm_pto_balances', {
    id: uuid('id').primaryKey().defaultRandom().notNull(),
    employeeId: uuid('employee_id')
        .notNull()
        .references(() => employees.id, { onDelete: 'cascade' }),
...
export const ptoLedgerEntries = pgTable('hrm_pto_ledger_entries', {
    id: uuid('id').primaryKey().defaultRandom().notNull(),
    employeeId: uuid('employee_id')
        .notNull()
        .references(() => employees.id, { onDelete: 'cascade' }),
