...
export function FullOrgChart({ apiPath, onNavigate }: FullOrgChartProps) {
  ...
  if (loading) {
    return (
      ...
      >
        <div style={{ color: '#64748b', fontSize: 14 }}>Loading organization chart...</div>
      </div>
    );
  }
  ...
  if (!roots.length) {
    return (
      ...
      >
        <div style={{ color: '#64748b', fontSize: 14 }}>No organization chart data available.</div>
      </div>
    );
  }
