...
export async function GET(request: NextRequest) {
  ...
  const sortOrder =
    String(url.searchParams.get('sortOrder') || 'desc').trim().toLowerCase() === 'desc' ? 'desc' : 'asc';
  ...
  const searchClause = search
    ? or(
        ilike(employees.preferredName, `%${search}%`),
        ilike(employees.firstName, `%${search}%`),
        ilike(employees.lastName, `%${search}%`),
        ilike(employees.userEmail, `%${search}%`),
        ilike(leaveTypes.name, `%${search}%`),
        ilike(ptoPolicies.name, `%${search}%`),
        ilike(ptoRequests.status as any, `%${search}%`)
      )
    : undefined;
...
export async function POST(request: NextRequest) {
  ...
  if (!employeeId) return jsonError('employeeId is required', 400);
  ...
  const resolvedPolicyId =
    policyIdRaw || (await resolvePolicyId({ request, employeeId: String(employee.id), userEmail: String(employee.userEmail || '') }));
  if (!resolvedPolicyId) {
  ...
  if (!row) return jsonError('Failed to create request', 500);
